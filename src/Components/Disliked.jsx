...
const DisLiked = () => {
  ...
  useEffect(() => {
    ...
    setDislikedVideos(users[currentUser]?.disliked || []);
  }, [currentUser]);

  const handleRemove = (id) => {
    const users = JSON.parse(localStorage.getItem('users')) || {};
    const userDisliked = users[currentUser]?.disliked || [];

    const updatedDisliked = userDisliked.filter((movie) => movie.id !== id);
    users[currentUser].disliked = updatedDisliked;
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-100 to-slate-200 py-8 px-6">
      ...
      <div className="max-w-7xl mx-auto">
        <h2 className="text-3xl font-bold text-gray-900 mb-8 text-center">Your Disliked Movies</h2>

        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-8">
          {dislikedVideos.length > 0 ? (
            dislikedVideos.map((item, ind) => (
              <div
                key={ind}
